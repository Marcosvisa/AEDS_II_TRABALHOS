...
time_t converterData(const char *dataStr) {
    ...
    if (sscanf(dataStr, "%s %d, %d", mes, &dia, &ano) == 3) {
        struct tm tm_data = {0};
        tm_data.tm_mday = dia;
        tm_data.tm_mon = mesParaNumero(mes) - 1; // tm_mon vai de 0 a 11
...
//metodo bolha para ordenar
void bubbleSort(Show *arr[], int n, int *comparacoes, int *trocas) {
    for (int i = 0; i < n - 1; i++) {
        for (int j = 0; j < n - i - 1; j++) {
            if (compareShows(arr[j], arr[j + 1], comparacoes) > 0) {
                // Troca
                Show *temp = arr[j];
                arr[j] = arr[j + 1];
                arr[j + 1] = temp;
...
int compareShows(Show *a, Show *b, int *comparacoes) {
    (*comparacoes)++;
    time_t ta = converterData(a->data);
    time_t tb = converterData(b->data);
...
int main() {
   ...
   while (!(strlen(entrada) == 3 && strcmp(entrada, "FIM") == 0)) {
       for (int x = 0; x < indice; x++) {
           if (strcmp(entrada, shows[x].id) == 0) {
               novo[i++] = &shows[x]; // já incrementa aqui
   ...
   bubbleSort(novo, i, &comparacoes, &trocas);
   ...
   FILE *log = fopen("812486_bolha.txt", "w");

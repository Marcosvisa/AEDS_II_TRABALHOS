...
// processar cada linha
 void processar(char s[], int i, Show shows[]) {
    ...
    // ler id
    while (s[x] != ',') id[strlen(id)] = s[x++];
    id[strlen(id)] = '\0';
    x++;

    // ler type
    while (s[x] != ',') type[strlen(type)] = s[x++];
    type[strlen(type)] = '\0';
    x++;
    ...
    if (s[x] == '"') {
        x++;
        while (s[x] != '"') title[strlen(title)] = s[x++];
    ...
    if (s[x] == '"') {
        x++;
        while (s[x] != '"') director[strlen(director)] = s[x++];
    ...
    if (s[x] == '"') {
        x++;
        while (s[x] != '"') country[strlen(country)] = s[x++];
    ...
    if (s[x] == '"') {
        x++;
        while (s[x] != '"') data[strlen(data)] = s[x++];
        data[strlen(data)] = '\0';
        x++;
    }
    while (s[x] == ',') x++;

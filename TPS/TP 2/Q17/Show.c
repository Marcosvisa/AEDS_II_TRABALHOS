...
void heapSortParcial(Show *arr[], int n, int k, int *comparacoes, int *trocas) {
...
void heapify(Show *arr[], int n, int i, int *comparacoes, int *trocas) {
    int largest = i;        
    int left = 2 * i + 1;     
    int right = 2 * i + 2;    
    if (left < n) {
        (*comparacoes)++;
        if (strcmp(arr[left]->type, arr[largest]->type) > 0 || 
           (strcmp(arr[left]->type, arr[largest]->type) == 0 && 
            strcmp(arr[left]->title, arr[largest]->title) > 0)) {
            largest = left;
        }
    }
    ...
    if (right < n) {
        (*comparacoes)++;
        if (strcmp(arr[right]->type, arr[largest]->type) > 0 || 
           (strcmp(arr[right]->type, arr[largest]->type) == 0 && 
            strcmp(arr[right]->title, arr[largest]->title) > 0)) {
            largest = right;
        }
    }
...
int main() {
  ...
  heapSortParcial(novo, i, 10, &comparacoes, &trocas);
   ...
   FILE *log = fopen("812486_heapSort_parcial.txt", "w");

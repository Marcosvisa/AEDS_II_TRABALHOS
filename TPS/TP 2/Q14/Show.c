...
/*bom, o algoritimo de ordenacao radixsort funciona verificando os digitos de um numero, 
percorrendo os digitos do numero da direita para a esquerda. Basicamente, fixa um verificador de 0 ate 9.
Por exemplo, quando o verificador é 0, todos os numeros que tiverem o ultimo digito sendo 0, 
sera jogado para o começo do array, e assim prosseguindo com o proximo digito á esquerda.
Por isso, ele é um algoritimo estavel
Sua complexidade Theta(nk), onde n é o numero de elementos e k a media de digitos dos elementos
 */

//o algoritimo esta no final dda classe 
...
int main() {
   ...
   while (!(strlen(entrada) == 3 && strcmp(entrada, "FIM") == 0)) {
       for (int x = 0; x < indice; x++) {
           if (strcmp(entrada, shows[x].id) == 0) {
               novo[i++] = &shows[x];
               break; //para evitar continuar procurando
           }
       }
       scanf(" %[^\n]", entrada);
   }
   ...
   radixSort(novo, i, &comparacoes, &trocas);
   ...
   FILE *log = fopen("812486_radixsort.txt", "w");
...
// Função para obter o maior ano
int obterMaiorAno(Show *arr[], int n) {
    int maior = arr[0]->ano;
    for (int i = 1; i < n; i++) {
        contarComp++;
        if (arr[i]->ano > maior) {
...
// Função de ordenação estável com base no dígito
void countingSortAno(Show *arr[], int n, int exp) {
    Show *saida[n];
    int count[10] = {0};

    for (int i = 0; i < n; i++) {
        int digito = (arr[i]->ano / exp) % 10;
        count[digito]++;
        contarComp++;
    }
...
// Função auxiliar para ordenar por título em caso de empate de ano
void desempatarPorTitulo(Show *arr[], int n) {
    for (int i = 0; i < n - 1; i++) {
        for (int j = 0; j < n - i - 1; j++) {
            contarComp++;
            if (arr[j]->ano == arr[j + 1]->ano &&
                strcmp(arr[j]->title, arr[j + 1]->title) > 0) {
                Show *temp = arr[j];
                arr[j] = arr[j + 1];
                arr[j + 1] = temp;
            }
...
void radixSort(Show *arr[], int n, int *comparacoes, int *trocas) {
    int maior = obterMaiorAno(arr, n);

    for (int exp = 1; maior / exp > 0; exp *= 10) {
        countingSortAno(arr, n, exp);
    }

    desempatarPorTitulo(arr, n);

    *comparacoes = contarComp;
    *trocas = 0; 

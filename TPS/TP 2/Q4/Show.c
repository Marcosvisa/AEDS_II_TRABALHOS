...
int main() {
   ...
   ordenarPorTitulo(novo, i);

   scanf(" %[^\n]", entrada);
   while (!(strlen(entrada) == 3 && strcmp(entrada, "FIM") == 0)) {
       if (buscaBinaria(novo, i, entrada, &comparacoes)) {
           printf("SIM\n");
       } else {
           printf("NAO\n");
       }
   ...
   FILE *log = fopen("812486_binaria.txt", "w");
...
void ordenarPorTitulo(Show *v[], int n) {
    for (int i = 0; i < n - 1; i++) {
        for (int j = 0; j < n - i - 1; j++) {
            if (strcmp(v[j]->title, v[j + 1]->title) > 0) {
           Show *temp = v[j];
                v[j] = v[j + 1];
                v[j + 1] = temp;
            }
...
int buscaBinaria(Show *v[], int n, char *alvo, int  *comp) {
    int esq = 0, dir = n - 1;
    while (esq <= dir) {
        int meio = (esq + dir) / 2;
        int cmp = strcmp(alvo, v[meio]->title);
        if (cmp == 0) return 1;

...
//selecao recursivo
void selectionSortRec(Show *arr[], int n, int index, int *comparacoes, int *trocas) {
    if (index == n) return;

    int min = index;
    for (int i = index + 1; i < n; i++) {
        (*comparacoes)++;
        if (strcmp(arr[i]->title, arr[min]->title) < 0) {
            min = i;
        }
    }
    ...
    selectionSortRec(arr, n, index + 1, comparacoes, trocas);
}
...
int main() {
   ...
   selectionSortRec(novo, i, 0, &comparacoes, &trocas);
   ...
   FILE *log = fopen("812486_selecaoRecursiva.txt", "w");

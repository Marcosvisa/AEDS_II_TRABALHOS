...
// metodo insertion sort para ordenar
void insert(Show *arr[], int n, int *comparacoes, int *trocas) {
    for (int i = 1; i < n; i++) {
        Show *temp = arr[i];
        int j = i - 1;
        ...
        while (
            ...
            (
                (*comparacoes)++,
                strcmp(temp->type, arr[j]->type) < 0 ||
                (
                    strcmp(temp->type, arr[j]->type) == 0 &&
                    strcmp(temp->title, arr[j]->title) < 0
                )
            )
        ) {
            arr[j + 1] = arr[j];
...
int main() {
   ...
   insert(novo, i, &comparacoes, &trocas);
   ...
   FILE *log = fopen("812486_insercao_parcial.txt", "w");

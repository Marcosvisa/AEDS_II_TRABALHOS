...
//metodo quicksort para ordenar
void quickSort(Show *arr[], int low, int high, int *comparacoes, int *trocas);

int compareShows(Show *a, Show *b, int *comparacoes) {
    (*comparacoes)++;
    int cmpType = strcmp(a->data, b->data);
    if (cmpType != 0) return cmpType;
    return strcmp(a->title, b->title);
}

int partition(Show *arr[], int low, int high, int *comparacoes, int *trocas) {
    Show *pivo = arr[high];
    int i = low - 1;

    for (int j = low; j < high; j++) {
        if (compareShows(arr[j], pivo, comparacoes) <= 0) {
            i++;
            Show *temp = arr[i];
            arr[i] = arr[j];
            arr[j] = temp;
...
int main() {
   ...
   quickSort(novo, 0, i - 1, &comparacoes, &trocas);
   ...
   FILE *log = fopen("812486_quicksort.txt", "w");
